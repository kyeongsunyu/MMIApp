...
namespace MMI
{
    public class CTcpServerAsync : CTcpBase
    {
        // Thread signal.
        ...
        public CTcpServerAsync(IPAddress ipAddr, int port, int maxClientCount = 100)
        {
        ...
        /// <param name="port">Port</param>
        /// <param name="maxClientCount">최대 클라이언트 수</param>
        public CTcpServerAsync(int port, int maxClientCount = 100)
        {
        ...
        {
            SetServerInfo(IPAddress.Any, port);
        }
        ...
        {
            CTcpServerAsync sv = (CTcpServerAsync)obj;
        ...
        public void Send(String data)
        {
            ...
            foreach (Socket socket in listClientSocket)
            {
                // Begin sending the data to the remote device.
                socket.BeginSend(byteData, 0, byteData.Length, 0,
                    new AsyncCallback(SendCallback), socket);
